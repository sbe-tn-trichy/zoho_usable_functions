...
| `scripts/inventory/generate_price_list.py` | Generate a CSV price list with minimum-margin protection | Resolves one required `--purchase-account` through `PURCHASE_ACCOUNT_IDS`, fetches only active items from Zoho Books, includes only `track_inventory=true` items, and exports valid rows as Group Name, Item Name, SKU, Cost Price, Sales Price, Margin, and ItemId. |
| `scripts/inventory/neoseal/Neoseal_items_normalizer.py` | Audit Neoseal item conventions and report anomalies | Rechecks item naming conventions, compares current grouping with the canonical product family, validates SKU formatting and terminology, and writes an anomaly-only CSV with one multiline detail column grouped by anomaly area. |
...
| `scripts/inventory/generate_neoseal_margin_analysis.py` | Generate Price and Margin Analysis report grouped by Item Group | Reads `neoseal_items_active.csv`, computes Selling Price, Cost Price, Margin (₹), and Margin (%), and exports CSV + multi-sheet Excel workbook. |
